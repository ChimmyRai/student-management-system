...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">Staff Training Details</div>

                <div class="card-body">
                  <div class="form-row mt-sm-2">
                        <div class="col-sm-3">
                            <button type="button"  wire:click.prevent="" class="btn btn-success mt-3" data-toggle="modal" data-target="#createModal">Add Training details</button>
                        </div>
                  <div class="col-sm-3">
                      <button type="button" wire:click=""  class="btn btn-success mt-3" data-toggle="modal" data-target="#uploadExcelModal">Upload Training details from Excel</button>
                  </div>
                  </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="createModalLabel">Training Details Form</h5>
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="form-group">
                        <label for="exampleFormControlInput3">Training Title<span class="mustFill">*</span></label>
                        <input type="text" wire:model="training_name" class="form-control" id="exampleFormControlInput3">
                        @error('training_name')<span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group">
                      <label for="exampleFormControlInput3">From<span class="mustFill">*</span></label>
                      <input type="date" wire:model="training_start_date" class="form-control" id="exampleFormControlInput3">
                      @error('training_start_date')<span class="text-danger">{{ $message }}</span>
                      @enderror
                    </div>

                    <div class="form-group">
                      <label for="exampleFormControlInput3">To<span class="mustFill">*</span></label>
                      <input type="date" wire:model="training_end_date" class="form-control" id="exampleFormControlInput3">
                      @error('training_end_date')<span class="text-danger">{{ $message }}</span>
                      @enderror
                    </div>
                    <div class="form-group">
                      <label for="exampleFormControlInput3">Attendence Type</label>
                     <input type="text" wire:model="attendence_type" class="form-control" id="exampleFormControlInput3">
                     @error('attendence_type')<span class="text-danger">{{ $message }}</span>
                     @enderror
                    </div>
...
<div>
  ...
  <table class="table table-bordered">
               <thead class="thead-light" style="position:sticky;top:0;">
                 ...
                 <th class="header">
                      <a wire:click.prevent="sortBy('training_name')" role="button" href="#">Title Of Training
                        @include('includes._sort-icon',['field'=>'training_name'])</a>
                    </th>

                <th class="header">
                    <a wire:click.prevent="sortBy('training_start_date')" role="button" href="#">From
                      @include('includes._sort-icon',['field'=>'training_start_date'])</a>
                </th>
                 <th class="header">
                   <a wire:click.prevent="sortBy('training_end_date')" role="button" href="#" class="nonToogleDisplay">To
                       @include('includes._sort-icon',['field'=>'training_end_date'])
                   </a>
                 </th>

                 <th class="header">
                      <a wire:click.prevent="sortBy('attendence_type')" role="button" href="#">Attendence Type
                        @include('includes._sort-icon',['field'=>'attendence_type'])</a>
                </th>
               ...
               <tbody>
                 @foreach($trainingDetailsList as $item)
                   <tr>
                     <td class="nonToogleDisplay"> <img src="{{url('storage/images/staff/'.$item->img_location) }}" class="datatablepic"></td>
                    <td>{{$item->Name}}</td>
                     <td>{{$item->cid}}</td>
                     <td>{{$item->training_name}}</td>
                      <td>{{\Carbon\Carbon::parse($item->training_start_date)->format('M d Y')}}</td>
                        <td>{{\Carbon\Carbon::parse($item->training_end_date)->format('M d Y')}}</td>
  ...
  {{$trainingDetailsList->links()}}

<div class="col text-right text-muted">
  showing {{$trainingDetailsList->firstItem()}} to {{$trainingDetailsList->LastItem()}} out of {{$trainingDetailsList->total()}}
</div>
         </div>
