...
class staffEmplloymentDetailsImport implements ToModel, WithHeadingRow, SkipsOnError, WithValidation, SkipsOnFailure, WithBatchInserts, WithChunkReading
{
...
return new staffEmploymentDetails([
          'cid'=>$row['cid'],
          'school'=>$row['school'],
          'dzongkhag_served'=>$row['dzongkhag_served'],
          'start_date'=>Carbon::instance(Date::excelToDateTimeObject(intval($row['start_date']))),
          'end_date'=>Carbon::instance(Date::excelToDateTimeObject(intval($row['end_date']))),
...
public function rules(): array
     {
         return [
             '*.cid' =>['required','numeric','unique:staff_employment_details,cid'],
             '*.school' =>['required'],
