...
class resultHigherComImport implements ToModel,  WithHeadingRow, SkipsOnError, WithValidation, SkipsOnFailure, WithBatchInserts, WithChunkReading
{
...
public function rules(): array
         {
             return [
                '*.student_code'=>['required','unique:result_higher_coms,student_code','regex:/^[0-9]+(\.[0-9]+)+$/'],
                ...
                '*.b_math' =>['required','min:0','max:100'],
                '*.commerce' =>['required','min:0','max:100'],
                '*.accountancy' =>['required','min:0','max:100'],
