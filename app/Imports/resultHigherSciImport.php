...
class resultHigherSciImport implements ToModel,  WithHeadingRow, SkipsOnError, WithValidation, SkipsOnFailure, WithBatchInserts, WithChunkReading
{
...
public function rules(): array
     {
         return [
            '*.student_code'=>['required','unique:resultmiddles,student_code','regex:/^[0-9]+(\.[0-9]+)+$/'],
            ...
            '*.math' =>['required','min:0','max:100'],
            '*.physics' =>['required','min:0','max:100'],
            '*.chemistry' =>['required','min:0','max:100'],
            '*.biology' =>['required','min:0','max:100'],
            ...
            '*.section'=>['required','string',Rule::exists('classteachers')->where(function ($query) {
                    return $query->where('user_id_of_teacher','=', Auth::user()->id);
                  })],
