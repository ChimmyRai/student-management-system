...
class staffTrainingDetailsImport implements ToModel, WithHeadingRow, SkipsOnError, WithValidation, SkipsOnFailure, WithBatchInserts, WithChunkReading
{
    ...
    */
    ...
    else
    {
      ...
      $user_name_of_updater=Auth::guard('admin')->user()->name;
        ...
        return new staffTrainingDetails([
              'cid'=>$row['cid'],
              'training_name'=>$row['training_name'],
              'training_start_date'=>Carbon::instance(Date::excelToDateTimeObject(intval($row['training_start_date']))),
              'training_end_date'=>Carbon::instance(Date::excelToDateTimeObject(intval($row['training_end_date']))),
...
public function rules(): array
             {
                 return [
                     ...
                     '*.training_end_date' =>['required'],
                     '*.attendence_type' =>['required','string'],
