...
class resultHigherComQueryExport implements FromQuery, WithHeadings,WithMapping,WithStyles
{
  use Exportable;
...
public function query()
                  {
                      return resultHigherCom::query()->where('student_code', 'like', '%'.$this->query.'%')
                                            ->orWhere('index_number', 'like', '%'.$this->query.'%')
                                            ->orWhere('email', 'like', '%'.$this->query.'%')
                                            ->orWhere('dzongkha', '=', $this->query)
                                            ->orWhere('english', '=', $this->query)
                                            ->orWhere('b_math', '=', $this->query)
                                            ->orWhere('commerce', '=', $this->query)
                                            ->orWhere('accountancy', '=', $this->query)
