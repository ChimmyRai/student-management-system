...
class resultHigherSciQueryExport implements FromQuery, WithHeadings,WithMapping,WithStyles
{
  use Exportable;
...
public function query()
       {
           ...
           return resulthighersci::query()->where('student_code', 'like', '%'.$this->query.'%')
                                 ->orWhere('index_number', 'like', '%'.$this->query.'%')
                                 ->orWhere('email', 'like', '%'.$this->query.'%')
                                 ->orWhere('dzongkha', '=', $this->query)
                                 ->orWhere('english', '=', $this->query)
                                 ->orWhere('math', '=', $this->query)
                                 ->orWhere('physics', '=', $this->query)
                                 ->orWhere('chemistry', '=', $this->query)
                                 ->orWhere('biology', '=', $this->query)
