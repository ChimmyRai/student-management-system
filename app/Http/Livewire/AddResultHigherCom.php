...
use App\Imports\resultHigherComImport;
use App\Exports\resultHigherComQueryExport;
class AddResultHigherCom extends Component
{
  use WithFileUploads;
  use WithPagination;
  ...
  public $selectedSection;
    ...
    {
      $this->resultClassList=resultHigherCom::select('class')->groupBy('class')->get();
      $this->resultSectionList=resultHigherCom::select('section')->groupBy('section')->get();
      $result=resultHigherCom::search($this->search)->where('user_id_of_updater','=',Auth::user()->id)->orderBy($this->sortfield, $this->sortDesc ? 'desc':'asc')->paginate($this->showperpage);
      return view('livewire.add-result-higher-com',['result'=>$result,'resultClassList'=>$this->resultClassList,'resultSectionList'=>$this->resultSectionList])->layout('layouts.sbadmin');
    }
    public function importExcel()
                {
                  ...
                  if(!empty($this->excelfile))
                  {
                    ...
                    $import=new resultHigherComImport;
...
//  dd($this->search,$this->sortfield, $sortOrder);
        return Excel::download(new resultHigherComQueryExport($this->search,$this->sortfield, $sortOrder), 'student_result_com.xlsx');
      }
...
public function emptyresult()
            ...
            // dd($this->selectedClass);
               $this->validate([
                ...
                $query=resultHigherCom::where('class',$this->selectedClass)->where('section',$this->selectedSection)->where('user_id_of_updater','=', Auth::user()->id)->delete();
                if($query){
