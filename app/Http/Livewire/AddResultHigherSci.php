...
use App\Imports\resultHigherSciImport;
use App\Exports\resultHigherSciQueryExport;
class AddResultHigherSci extends Component
{
  use WithFileUploads;
  use WithPagination;
...
public function render()
    {
      $this->resultClassList=resulthighersci::select('class')->groupBy('class')->get();
      $this->resultSectionList=resulthighersci::select('section')->groupBy('section')->get();
      $result=resulthighersci::search($this->search)->where('user_id_of_updater','=',Auth::user()->id)->orderBy($this->sortfield, $this->sortDesc ? 'desc':'asc')->paginate($this->showperpage);
      return view('livewire.add-result-higher-sci',['result'=>$result,'resultClassList'=>$this->resultClassList,'resultSectionList'=>$this->resultSectionList])->layout('layouts.sbadmin');
    }
public function importExcel()
            {
              ...
              if(!empty($this->excelfile))
              {
                ...
                $import=new resultHigherSciImport;
      ...
      public function exportExcel()
              {
                if($this->sortDesc)
                {
                  ...
                  return Excel::download(new resultHigherSciQueryExport($this->search,$this->sortfield, $sortOrder), 'student_result_sci.xlsx');
                }
...
public function emptyresult()
        ...
        // dd($this->selectedClass);
           ...
           $query=resulthighersci::where('class',$this->selectedClass)->where('section',$this->selectedSection)->where('user_id_of_updater','=', Auth::user()->id)->delete();
           if($query){
